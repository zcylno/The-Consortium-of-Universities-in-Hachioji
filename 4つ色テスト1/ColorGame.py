...
class Rect_Model():
    ...
    def update(self):        
        if self.model.x <= 0: self.model.x = 0
        elif self.model.x >= (W-self.size.x): self.model.x = W - self.size.x
        if self.model.y <= 0: self.model.y = 0
        elif self.model.y >= (H-self.size.y): self.model.y = H - self.size.y
    ...
    def move_(self, action):
        if self.moving:
            ...
            if action==3:
                self.model.x -= 30
def game_init():
    global screen, backbuffer, timer, farmer, cell1,cell2,cell3,cell4,cell5,cell6,cell7,cell8, H, W, count_cash, size,c,c1,c2,c3,c4
    ...
    cell1_pos = Point(W/2-size,0)
    cell1 = Rect_Model('cell1', (255,0,0), cell1_pos)
    cell2_pos = Point(0,H/2-size)
    cell2 = Rect_Model('cell2', (0,255,0), cell2_pos)
    cell3_pos = Point(W,H/2-size)
    cell3 = Rect_Model('cell3', (0,0,255), cell3_pos)
    cell4_pos = Point(W/2-size,H)
    cell4 = Rect_Model('cell4', (50,50,50), cell4_pos)
...
def set_cash(count):
    ...
    cell1.count_cash = cell2.count_cash = cell3.count_cash = cell4.count_cash = 0



def plt_cash():
    Tools.plot(c, c1,c2,c3,c4)
    c1.clear()
    c2.clear()
    c3.clear()
    c4.clear()
